...
public:
	...
	FeatureInput(int sr = 16000, short h = 160) :fs(sr), hop(h) {}
	void compute_f0(const double* audio, int64_t len);
	void resize2d_f0(int64_t len);
	long long* coarse_f0() const;
	std::vector<long long> transcribe(const double* audio, int64_t audioLen, int64_t hubLen, int64_t tran);
	std::vector<float> transcribe3(const double* audio, int64_t audioLen, int64_t hubLen, int64_t tran);
	V4Return transcribe4(const double* audio, int64_t audioLen, int64_t tran);
	std::vector<float> transcribeF0(const double* audio, int64_t audioLen, int64_t tran);
