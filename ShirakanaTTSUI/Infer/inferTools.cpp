...
void conArr2Wav(long long size, short* input, const char* filename, int samp)
{
    WavHead head = { {'R','I','F','F'},0,{'W','A','V','E'},{'f','m','t',' '},16,
            1,1,samp,samp * 2,2,16,{'d','a','t','a'},
            0 };
...
void FeatureInput::compute_f0(const double* audio, int64_t len)
{
	DioOption Doption;
...
void FeatureInput::resize2d_f0(int64_t len)
{
...
long long* FeatureInput::coarse_f0() const
{
...
std::vector<long long> FeatureInput::transcribe(const double* audio, int64_t audioLen, int64_t hubLen, int64_t tran)
{
	...
	resize2d_f0(hubLen);
	const auto O0f = coarse_f0();
	std::vector<long long> Of0(O0f, O0f + f0Len);
	return Of0;
}

std::vector<float> FeatureInput::transcribe3(const double* audio, int64_t audioLen, int64_t hubLen, int64_t tran)
{
	compute_f0(audio, audioLen);
	for (int64_t i = 0; i < f0Len; ++i)
	{
...
FeatureInput::V4Return FeatureInput::transcribe4(const double* audio, int64_t audioLen, int64_t tran)
{
...
std::vector<float> FeatureInput::transcribeF0(const double* audio, int64_t audioLen, int64_t tran)
{
	compute_f0(audio, audioLen);
	for (int64_t i = 0; i < f0Len; ++i)
	{
    ...
    if (!c.empty() && !d.empty() && !e.empty())
    {
        const auto f = new double[c.size()];
        interp1(d.data(), e.data(), static_cast<int>(d.size()), c.data(), static_cast<int>(c.size()), f);
        for (size_t i = 0; i < c.size(); ++i)
            rf0[static_cast<size_t>(c[i])] = f[i];
        delete[] f;
    }
    std::vector<float> Of0(specLen, 0.0);
	for (int64_t i = 0; i < specLen; ++i)
