...
F0PreProcess::RVCR F0PreProcess::GetF0AndOtherInputR(const double* audio, int64_t audioLen, int64_t hubLen, int64_t tran)
{
    compute_f0(audio, audioLen);
    for (int64_t i = 0; i < f0Len; ++i)
    {
    ...
    InterPf0(hubLen);
    std::vector<float> f0(f0Len);
    for (int64_t i = 0; i < f0Len; ++i)
        f0[i] = static_cast<float>(rf0[i]);
    const auto O0f = f0Log();
    std::vector<long long> Of0(O0f, O0f + f0Len);
    delete[] O0f;
    return { std::move(f0),std::move(Of0) };
}
